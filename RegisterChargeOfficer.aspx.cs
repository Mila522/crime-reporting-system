...
{
    public partial class RegisterChargeOfficer : Page
    {
        protected void btnRegister_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                using (var context = new AppDbContext())
                {
                    ...
                    // Add user to ChargeOfficer role
                    if (!Roles.RoleExists("ChargeOfficer"))
                        Roles.CreateRole("ChargeOfficer");
                    if (!Roles.IsUserInRole(username, "ChargeOfficer"))
                        Roles.AddUsersToRoles(new[] { username }, new[] { "ChargeOfficer" });

                    lblMessage.Text = "Registration successful. Please log in.";
                    System.Diagnostics.Debug.WriteLine($"User registered: {username}, Role=ChargeOfficer");
